...
# Extract the TF-IDF scores for all spam emails
spam_emails = X_train[Y_train == 0]
spam_features = feature_extraction.transform(spam_emails)

# Aggregate the TF-IDF scores for each word
feature_names = feature_extraction.get_feature_names_out()
tfidf_scores = np.sum(spam_features.toarray(), axis=0)
...
# Plot the top words by their aggregate TF-IDF scores
plt.figure(figsize=(12, 8))
plt.barh(tfidf_df['word'], tfidf_df['tfidf'], color='skyblue')
plt.xlabel('TF-IDF Score')
plt.ylabel('Words')
plt.title('Words in Spam Emails by TF-IDF Score')
plt.gca().invert_yaxis()
plt.show()

inputs="""
Halo Kevin ini Rian, km coba cek ini disitu ada dokumentasi buat code python yang dicari
"""
